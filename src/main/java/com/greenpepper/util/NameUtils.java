...
public final class NameUtils
{
    ...
    /**
     * Tries to convert <code>string</code> to a Java class name by following
     * a few simple steps:
     * <p/>
     * <p/>
     * <ul>
    ...
    {
        if (StringUtil.isEmpty( s ) || s.contains( "." )) return s;
    ...
    public static String decapitalize( String s )
    {
        ...
        for (int i = 1; i < parts.length; ++i)
        {
            sb.append( capitalizeFirst( Introspector.decapitalize( parts[i] ) ) );
        }
        return sb.toString();
    }
